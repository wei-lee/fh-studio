...
(function ($) {
	...
	$.jstree.plugin("themes", {
		__init : function () { 
			this.get_container()
				.bind("init.jstree", $.proxy(function () {
						...
						//alert(s.dots);
						this.set_theme(s.theme, s.url, s.loaded);
					}, this))
		...
		_fn : {
			set_theme : function (theme_name, theme_url, theme_loaded) {
...
(function ($) {
	$.vakata.context = {
		...
		show	: function (s, t, x, y, d, p) {
			var html = $.vakata.context.parse(s, false, p), h, w;
			if(!html) { return; }
		...
		parse	: function (s, is_callback, obj) {
			if(!s) { return false; }
			var str = "",
				tmp = false,
				was_sep = true;
			...
			$.each(s, function (i, val) {
				...
				was_sep = false;
				var disabled = $.isFunction(val._disabled) ? val._disabled(obj) : val._disabled;
				str += "<li class='" + (val._class || "") + (disabled ? " jstree-contextmenu-disabled " : "") + "'><ins ";
				...
				str += val.label + "</a>";
				if(val.submenu) {
					tmp = $.vakata.context.parse(val.submenu, true, obj);
					if(tmp) { str += tmp; }
				}
				str += "</li>";
