...
Admin.Authpolicy.Controller = Controller.extend({
  ...
  form_template: ['<form class="well">', '<h3>%ACTION_CAP% an Authentication Policy</h3>', '<p>Here you can %ACTION% an authentication policy which can be used by your applications.</p>', '<br/>', '<label>Policy Id</label>', '<input id="policy_id" type="text" class="span6" placeholder="Policy Id">', '<label>Type</label>', '<label class="select">', '<select class="" id="policy_type">', '<option value="oauth"> oAuth </option>', '</select>', '</label>', '<label>Provider</label>', '<label class="select">', '<select id="policy_conf_provider">', '<option value="google">Google</option>', '</select>', '</label>', '<label>Client Id</label>', '<input id="client_id" type="text" class="span6" placeholder="Client Id (provided by Oauth service (ie Google etc...))">', '<label>Client Secret</label>', '<input id="client_secret" type="text" class="span6" placeholder="Client Secret (provided by Oauth service (ie Google etc...))">', '<label>Oauth Callback Url (Note* this should be added as the oauth callback url on the provider side i.e Google)</label>', '<input id="oauth_callback" type="text" class="span6" disabled="disabled">', '<label>Users (Use comma to separate multiple users)</label>', '<label class="textarea">', '<textarea id="policy_users" rows=10></textarea>', '</label>', '<button type="button" class="btn btn-primary edit_policy_btn">%ACTION_CAP% Policy</button>', '</form>'],
  ...
  createPolicy: function(action) {
    ...
    this.models.policy[action](id, type, conf, {
      list: usersArr
    }, function(res) {
      //console.log(res);
      self.showPolicyList();
