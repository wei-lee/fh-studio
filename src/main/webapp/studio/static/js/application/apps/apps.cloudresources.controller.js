...
Apps.Cloudresources.Controller = Apps.Cloud.Controller.extend({
  ...
  showSample: function() {
    var history = $('<div>').addClass('history_container').attr('id', 'cloud_memory_history');
    $('#cloud_memory').append(history);
    var chart = new Highcharts.Chart({
      chart: {
        renderTo: 'cloud_memory_history',
        zoomType: 'x',
        spacingRight: 20
      },
      title: {
        text: 'Memory Usage (Monthly)'
      },
      xAxis: {
        type: 'datetime',
